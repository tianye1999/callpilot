(() => {
  ...
  async function requestInvite() {
    if (!pairedEdgeId) throw new Error("edge unavailable");
    const { response, payload } = await postJson("/v1/calls", {
      edgeId: pairedEdgeId,
      idempotencyKey: idempotencyKey(),
    });
    if (!response.ok || typeof payload.callId !== "string") {
      throw new Error(payload.error?.code || "session unavailable");
    }
    const deadline = Date.now() + 12000;
    while (Date.now() < deadline) {
      const statusResponse = await fetch(`/v1/calls/${encodeURIComponent(payload.callId)}`, {
        cache: "no-store",
      });
  ...
  async function pairPhone() {
    ...
    setStatus("pairing");
    try {
      const { response, payload } = await postJson("/v1/pairing-sessions/claim", { code, displayName: name });
      if (!response.ok || !payload.device) {
        setStatus("failed", payload.error || t("pair_failed"));
        return;
      }
      paired = true;
      pairedEdgeId = payload.device.edgeId;
      terminal = false;
  ...
  async function unpairPhone() {
    ...
    try {
      await fetch("/v1/device", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: "{}",
        cache: "no-store",
      });
    } catch (_error) { /* clear local view below */ }
    paired = false;
    pairedEdgeId = null;
    terminal = false;
    invite = null;
  ...
  if ("serviceWorker" in navigator) {
    navigator.serviceWorker.register("/remote_dialer_sw.js?v=3", {
      updateViaCache: "none",
    }).catch(() => {});
  }
  initialize();
