...
- **P3-1 DTMF 按键 —— 🚧 信号层已实现、真机端到端待验证**：带内合成（默认 `DTMF_MODE=inband`）/ EC20 `AT+QVTS` 回退 `AT+VTS`（打客服菜单导航）；真机菜单推进有效性见 #73/#80，待 A/B 实测收口。
- **P3-2 barge-in（打断）**：realtime 原生支持，当前被半双工压制；依赖 §4 S2 回声结论决定放开策略。
- **P3-3 音频治理**：下行 AGC 已实现（`MODEM_AGC` / `MODEM_AGC_TARGET_DBFS`，见 `docs/sim7600-windows-inbound-debug.md` §5.2）；仍缺**上行** AGC、削波检测、静音填充参数化。
...
- **真机验收清单**（`docs/acceptance.md`，待补）：来电自动接听×3、qwen 对话 5 轮×3 通、外呼×1、
...
- **外呼本卡运营商客服 IVR 验证双向语音链路**（按 SIM 识别，当前移动卡=**10086**；电信 10000/联通 10010）：客服 IVR 多为语音识别菜单，能与之**多轮
  ...
  *触发*：`curl -X POST http://127.0.0.1:47100/api/call/dial -d '{"number":"10086"}'`（号码按本卡运营商，当前移动=10086，勿跨网拨他家客服）。
- **发短信"查余额"到本卡运营商客服号验证短信收发**（当前移动=10086）：发出后运营商会回一条余额短信 →
...
- **部署文档**（`docs/deploy-macos.md`，待补；当前安装见 README 与 docs/faq.md）：从零到可接电话（Python/依赖/USB 桥/常驻）；
  key 管理规范；Linux 差异预留。
...
## 附录 B：短信转发到邮箱（已实现，issue #17）

- [x] `+CMTI→on_sms→EventHub sms_in` 先维持原短信落盘/界面链路，再非阻塞入有界邮件队列；
  只处理实时新短信，不重放 `messages.json` 历史。
- [x] 移植并收紧旧 POC 的 OTP 识别：只有验证码语境附近的候选值才进入邮件标题，普通数字不误报。
- [x] 设置面板提供默认关闭的转发开关、单一收件邮箱和每部署自备的 SMTP 参数；跨字段校验原子落盘，
  密码按 `secret` 掩码且不进日志/API 响应。
- [x] SMTP 仅允许证书校验的 STARTTLS/SSL，后台发送有限重试；失败不阻塞模组/通话，也不记录正文、
  验证码或凭证。公开 DMG/EXE 不内置共享发件密码。
