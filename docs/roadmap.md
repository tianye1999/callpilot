...
>
> **状态更新（v0.2，2026-07-08）**：下方 FR-7/9/10/11（桌面 App、批量外呼、通话记录/录音、
> 通话后摘要）及 §3 对应的 P0-2/P1-1/P1-4/P2-1/P3-8 均已落地；provider 由 qwen/doubao 扩展为
> **qwen（默认）/ doubao / openai**。§3 明细里的状态标记尚未逐条回填，以 FR 表与本状态说明为准。
...
| FR-7 | **交付为桌面 App**（非浏览器网页） | ✅ 已实现（`desktop_app.py` + macOS 托盘 `tray_app.py`；v0.3 独立安装包仍待做） |
...
- **NFR-3 安全**：工具调用有 policy——发短信白名单+频控、挂断需显式意图+延迟窗、
  ...
  > ⚠️ **实现状态（2026-07-08 代码审计，防止文档领先代码）**：本条 6 项里**仅「挂断显式意图+延迟窗」已落地**；
  > 其余为 TODO，勿视为已交付——发短信白名单（`SMS_WHITELIST` 已注册但两发送入口 `web/server.py`、
  > `call_tools.py` 均未强制）、发短信频控、查验证码可配开关（工具当前无条件启用）、短信内容脱敏 4 项**均未实现**；
  > 工具调用审计日志仅 `send_dtmf` 有 `log_event`，其余工具无。
- **NFR-4 隐私**：录音/转写默认本地存储、可一键清除；`.env` 与各类凭证不入 git。

  > ⚠️ **实现状态**：本地存储 ✅、`.env` 不入 git ✅；但**「一键清除」未实现**（仅有按
  > `RECORDING_RETENTION_DAYS` 的自动过期清理，无手动清除端点/按钮），列 TODO。
...
- **P3-10 动态场景提示词生成**（later，任务类型丰富后再做）：按（拨打号码+任务）用便宜文本模型（qwen-plus）为每通生成贴合的场景描述，替代单一通用模板，延续「讲场景不列规则」思路。硬约束：
  1. **不变量固定、不交给生成**：立场（机主名下的事、对方是协助方）、安全边界、可用工具、"办完即收尾" 作为固定框架；只让模型生成"这通大概什么场景、怎么开口"那部分。
  2. **当前模板永远作默认/兜底**：生成失败或超时即回退到模板。
  3. **延迟藏进拨号窗口**：生成放在外呼 ~45s 接通等待期内做、按（号码+任务）缓存，不加感知延迟。
  4. **可调试**：每通生成的提示词记入通话日志，AI 抽风时能回看当时喂了什么。
  - 收益在任务多样化（催办/预约/客服协商…）后才明显；当前多为"打运营商 IVR 查询"，场景模板已够用。
- **P3-11 未拿到实质结果前不接受收场**（待做，务必非枚举实现）：真机见过对方只客套"还有别的吗"、并未给出所要结果，AI 却跟着收场（收尾裁判已从"是否拿到实质结果"把关、不会误判达成；但 AI 自身仍会附和收场）。要让 AI 在结果到手前主动把话题拉回、追问结果。**实现约束：用场景/目标表述（"你要的是实质结果，没拿到别收场"）让模型自己判断，绝不要枚举挂起语/客套话术的词表**——否则宁可不做。
...
（ruff/mypy、CI、.env.example 绑定测试）→ P3 打磨 + 5 个高 ROI 测试缺口。
...
- **D7 工具安全 policy（NFR-3）**：发短信白名单+频控、挂断显式意图+延迟窗、查验证码可配、审计日志。
  **（实现状态见上 NFR-3 审计注：截至 2026-07-08 仅「挂断延迟窗」落地，余为 TODO）**
