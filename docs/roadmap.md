...
**P0-0 【最高】验证并打通"下行到电话端可闻"**
- 现状（已验证事实）：**上行通**（电话→AI，18:33 拨 10000 其 IVR 语音被 qwen 准确转写，
  证明 `AC Interface` 采集正常）；**下行（AI→电话端可闻）从未被验证**——18:33 对端是 IVR
  机器人不会回应，而有真人在线的时刻反馈均为"没声音"。之前"全链路打通"的说法过头，
  实际只证明了上行 + 不崩溃。
- 开放问题：AI 音频经 ffmpeg 写到 `AS Interface`(audiotoolbox 输出)后，是否真的作为通话
  下行送到对端并可闻？两种模式（uac_ffmpeg / nmea）**都未验证**。
- 要做：① 真人在电话另一端的最小验证（uac_ffmpeg 模式拨真人号码，确认能否听到 AI）；
  ② 若听不到，排查 EC20 UAC 下行路由/增益（AS Interface 是否真接入通话下行、tx_gain）；
  ③ 借助 P2-7 的 Mac 扬声器监听把"AI 有没有出声"与"电话端有没有收到"分离定位。
- 验收：真人电话端清晰听到 AI 说话（非 IVR、非转写，是真人耳朵确认）。
- 备注：nmea 曾被怀疑是下行可闻的关键，但无实证且会崩 USB（见 §6 D3）；不作为首选，
  仅在 uac_ffmpeg 下行确证失败后，作为"先修 USB 崩溃再验证"的备选路径。
...
- **D3 音频模式（截至目前的实测状态，注意下行未验证）**：
  ...
  - 已证实：`uac_ffmpeg`（ffmpeg 绕过 PortAudio）**稳定不崩**，且**上行通**（拨 10000 其 IVR
    被 qwen 准确转写 → AC Interface 采集正常）。
  - **未证实（P0-0 开放问题）**：任何模式下 **AI 下行到电话端是否可闻**都尚未由真人在电话端
    确认过。曾有观点认为 nmea 才是下行可闻关键，但无实证、且 nmea 崩 USB，故不作首选。
  - 现结论：`uac_ffmpeg` 为**当前默认与最稳路径**（不崩 + 上行确证）；下行可闻性待 P0-0 验证；
    `uac`/`nmea` 保留为诊断/备用。Windows/Linux 原生串口另行验证。
  - ⚠️ 纠错：之前"全链路打通/uac_ffmpeg 唯一可用"的表述过头——只证明了上行与不崩溃，
    未证明电话端听到 AI。
