...
| **v0.3 One-click Mac Beta** | `.dmg` 安装、首次启动向导、内置 runtime、自动 launchd、Developer ID 签名+公证 | 普通用户 | 🚧 安装包+向导+runtime+launchd 已做；签名/公证仍缺 |
...
推荐路径与 per-user Application Support 数据目录处理。仍缺硬件兼容矩阵完善、Developer ID
签名与 notarization。
...
| FR-7 | **交付为桌面 App**（非浏览器网页） | ✅ 已实现（`desktop_app.py` + 托盘 `tray_app.py` + 独立安装包 `packaging/build_installer.sh`；签名公证仍缺） |
...
- **P3-11 未拿到实质结果前不接受收场 —— ✅ 已完成**：已通过目标/场景提示词与
...
- **P3-12 抑制对机器 IVR 的重复应答**（结构性，非提示词/非枚举）：真机现象——对方是"一直播报"的自动语音时，server VAD 把它切成很多轮，实时模型每轮都被迫回话、没新内容就复读开场白（"您好我是X想咨询…"连说 5-6 遍）。**提示词层面压不住**（属模型对机器 IVR 的轮次兜底行为，非措辞问题）。候选结构性方案：
  ...
  2. **改用手动应答控制 —— 待做**：关掉 server_vad 自动 create response，由我们判断"对方是否说了有意义的内容、AI 该不该开口"再触发 response.create——控制力最强、能根治，但改动大且要重新标定正常对话的节奏。
  3. **调 VAD 转向阈值**减少对连续播报的过度切分——副作用：拖慢真人对话响应。
  - 倾向 ②为长期正解；②落地前 ①可作轻量缓解。
- **P3-13 核实 send_dtmf 真机端到端有效**（待测）：日志显示 AI 确实调用了 `send_dtmf`（`已发送 DTMF: 1/0`，走 AT+QVTS，失败回退 AT+VTS），但某次观察中用户本人也手动按了同一键，无法确认是 AI 的按键推动了 IVR、还是用户的。需在**用户完全不手动干预**下单独测：AI 的 send_dtmf 能否真正让对端 IVR 菜单前进（DTMF 音是否被运营商识别）。若无效，排查 QVTS/VTS 在通话语音通道上的实际发音。
